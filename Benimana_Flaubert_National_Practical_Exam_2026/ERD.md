...
Database: **VRS** (MongoDB / Mongoose)

- Each entity is a **collection**; relationships are by **reference** (the child stores
  the parent's key), joined at read time with an aggregation `$lookup`.
- **PK** = the document's logical key, **FK** = the referenced key. Integer keys are
  generated by a `counters` collection (MongoDB has no `AUTO_INCREMENT`).
